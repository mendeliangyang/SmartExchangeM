...
public class CoordinateConversion {
    ...
    public BigDecimal[] baidulatlon2UTM(double latitude, double longitude) {
        BaiduLatLon2UTM dd = new BaiduLatLon2UTM();
        return dd.LatLon2UTM(latitude, longitude);
    }
    ...
    private class BaiduLatLon2UTM {

        int[] yt = new int[]{75, 60, 45, 30, 15, 0};
        double[][] zf = new double[][]{{-0.0015702102444, 111320.7020616939, 1704480524535203.0, -10338987376042340.0, 26112667856603880.0, -35149669176653700.0, 26595700718403920.0, -10725012454188240.0, 1800819912950474.0, 82.5},
        ...
        public BigDecimal[] LatLon2UTM(double lat, double lng) {
            if (lat == 0.0 || lng == 0.0 || 180 < lng || -180 > lng
                    || 90 < lat || -90 > lat) {

                return new BigDecimal[]{new BigDecimal("0.0"), new BigDecimal("0.0")};
            }

            lng = accessData(lng, -180, 180);
            lat = choose(lat, -74, 74);

            double[] c = new double[]{};
            for (int d = 0; d < yt.length; d++) {
                if (lat >= yt[d]) {
                    c = zf[d];
                    break;
                }
            ...
            double[] result = sj(new double[]{lat, lng}, c);
            if (result.length > 1) {
                DecimalFormat df = new DecimalFormat("0.00");

                return new BigDecimal[]{new BigDecimal(df.format(result[0])),
                    new BigDecimal(df.format(result[1]))};
            }
            return null;
        }

        private Double accessData(double a, double b, double c) {
            for (; a > c;) {
                a -= c - b;
            }
            for (; a < b;) {
        ...
        private double choose(double a, double b, double c) {
            a = a > b ? a : b;
            a = a < c ? a : c;
            return a;
        }

        private double[] sj(double[] a, double[] b) {
            if (a.length == 0 || b.length == 0) {
                return null;
            }
            double c = b[0] + b[1] * Math.abs(a[1]);
            double d = Math.abs(a[0]) / b[9];
