    ...
    public static JSONObject LocationDataAccess1(JSONArray jsonArrays, String latKey, String lngKey) {
        ...
        for (int level = 10; level <= 19; level++) {
            ...
            for (Object jsonArray : jsonArrays) {
                ...
                String tempValue = jsonObj.getString(latKey);
                jsonObj.remove(latKey);
                jsonObj.accumulate(latKey, tempValue);//replace
                tempValue = jsonObj.getString(lngKey);
                jsonObj.remove(lngKey);
                jsonObj.accumulate(lngKey, tempValue);
                //平面坐标
